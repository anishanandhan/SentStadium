...
def test_reset_data(client: TestClient) -> None:
...
"""
    ...
    response = client.post("/api/data/upload", files={"file": ("test.csv", file_obj, "text/csv")})
...
def test_upload_invalid_mime(client: TestClient) -> None:
    file_obj = io.BytesIO(b"random bytes")
    response = client.post("/api/data/upload", files={"file": ("test.txt", file_obj, "text/plain")})
